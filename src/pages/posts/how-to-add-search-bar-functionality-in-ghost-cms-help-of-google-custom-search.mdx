...
description: In ghost CMS, There are many ways to add search bar functionality in ghost
  CMS. But most commonly use Google's custom search tool to add a search bar.
draft: false
...
Ghost create own in built search functionality, Now this article is outdated {.info}
