...
description: Install deno in linux with easy ways.
...
After Installation is complete, restart your laptop or pc. {.info}
...
You can also remove the deno folder with `which deno` command `sudo rm -R $( which deno )` {.info}
...
Remove configuration in `.bashrc` file and suggest reopening the terminal or restarting your system. {.info}
