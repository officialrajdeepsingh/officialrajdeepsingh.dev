---
draft: false
title: What limitations to using tauri and nextjs together, and frontend developers build an Application without knowledge of Rust language?
description: I answer the most important two questions, which you know before starting with nextjs and tauri, in a straightforward format.
...
Tauri uses `npm run build && npm run export` commands to generate a static site. You cant use `getServerSideProps` in nextjs. You can only use `getStaticProps` in tauri. For data fetching, tauri provides an [https API](https://tauri.app/v1/api/js/http) for that. If You enable backend service in your tauri application, then Rust is essential.
